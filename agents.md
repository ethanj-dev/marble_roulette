...
- Custom map route width rules are split: a passage only needs `MIN_CUSTOM_PASSAGE_WIDTH = BALL_RADIUS * 2 + 24` to be valid, but pins/bumpers/exploders/rotating bars require `MIN_CUSTOM_OBSTACLE_PATH_WIDTH = BALL_RADIUS * 2 + 76`.
...
- `MIN_CUSTOM_PASSAGE_WIDTH = BALL_RADIUS * 2 + 24` lets narrow custom routes save when a ball can pass.
...
- 2026-06-11: Ran 1,000,000 randomized custom width/rotating-bar rule checks; passed with passage threshold 44px, obstacle threshold 96px, wall-attachment threshold 128px, `wallAttachAllowed=515218`, `wallAttachRejectedBelowThreshold=484782`, and no failures.
